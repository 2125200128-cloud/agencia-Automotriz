...
<section class="vm-page-section">
    <div class="vm-container-lg">
        ...
        <div class="vm-card-form">
            <form action="/proveedor/{{ $proveedor->id }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                @csrf
                @method('PUT')
                <div class="grid gap-6 md:grid-cols-2">
                    ...
                    <div><label class="vm-input-label" for="estado">Estado</label><select class="vm-input-text" id="estado" name="estado" required><option value="activo" @selected(old('estado', $proveedor->estado) === 'activo')>Activo</option><option value="inactivo" @selected(old('estado', $proveedor->estado) === 'inactivo')>Inactivo</option></select></div>
                    <div class="md:col-span-2"><label class="vm-input-label" for="imagen">Imagen</label>@include('plantilla.imagen-tabla', ['imagen' => $proveedor->imagen, 'alt' => $proveedor->nombre])<input class="vm-input-text mt-3" type="file" id="imagen" name="imagen" accept="image/*"></div>
