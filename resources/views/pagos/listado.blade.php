        ...
        <div class="mx-auto max-w-screen-2xl">
            ...
            <div class="mt-8 rounded-xl border border-gray-200 bg-white p-5">
                <div class="grid gap-4 lg:grid-cols-[1fr_200px_200px_auto]">
                    <input type="search" placeholder="Buscar por pedido, cliente o referencia..."
                            ...
                            @forelse ($pagos as $pago)
                                <tr class="hover:bg-gray-50 transition">
                                    <td class="px-6 py-4 font-bold text-black">
                                        PAY-{{ str_pad($pago->id, 5, '0', STR_PAD_LEFT) }}</td>
                                    <td class="px-6 py-4">#ORD-{{ str_pad($pago->pedido_id, 4, '0', STR_PAD_LEFT) }}</td>
                                    <td class="px-6 py-4">{{ $pago->pedido?->cliente?->nombres }}
                                        {{ $pago->pedido?->cliente?->apellidos }}</td>
