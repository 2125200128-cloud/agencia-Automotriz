        ...
        <div class="vm-card-form">
            <form action="/producto/{{ $producto->id }}" method="POST" enctype="multipart/form-data" class="space-y-8">
                ...
                <div class="grid gap-6 md:grid-cols-2">
                    ...
                    <div><label class="vm-input-label" for="estado">Estado</label><input class="vm-input-text" id="estado" name="estado" value="{{ old('estado', $producto->estado) }}" required></div>
                    <div><label class="vm-input-label" for="imagen_principal">Imagen principal</label><input class="vm-input-text" type="file" id="imagen_principal" name="imagen_principal" accept="image/*"><p class="mt-2 text-xs text-gray-400">{{ $producto->imagen_principal ?? 'Sin imagen actual' }}</p></div>
                    <div><label class="vm-input-label" for="imagen_secundaria">Imagen secundaria</label><input class="vm-input-text" type="file" id="imagen_secundaria" name="imagen_secundaria" accept="image/*"><p class="mt-2 text-xs text-gray-400">{{ $producto->imagen_secundaria ?? 'Sin imagen actual' }}</p></div>
                    <div><label class="vm-input-label" for="imagen_adicional">Imagen adicional</label><input class="vm-input-text" type="file" id="imagen_adicional" name="imagen_adicional" accept="image/*"><p class="mt-2 text-xs text-gray-400">{{ $producto->imagen_adicional ?? 'Sin imagen actual' }}</p></div>
                </div>
