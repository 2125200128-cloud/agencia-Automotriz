    ...
    <section class="vm-page-section">
        <div class="vm-container-md">
            <div class="vm-card-form">
                <p class="vm-header-tag">Eliminar tipo</p>
                <h1 class="vm-header-title">{{ $tipo->nombre }}</h1>
                <p class="mt-4 text-gray-400">Confirma si deseas eliminar este registro.</p>
                <form action="/tipos/{{ $tipo->id }}" method="POST" class="mt-8 flex justify-end gap-3">@csrf
                    @method('DELETE')<a href="/tipos" class="vm-btn-secondary">Cancelar</a><button type="submit"
