        ...
        <div class="vm-container-2xl">
            <div class="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
                <div class="vm-page-header">
                    ...
                    <p class="vm-header-desc">Listado interno para controlar accesos, roles y estado del equipo
                        administrativo.</p>
                </div>
                        ...
                        <tbody>
                            @forelse ($administradores as $administrador)
                                <tr class="vm-table-tr">
                                    ...
                                    <td class="vm-table-td font-bold text-black">{{ $administrador->nombres }}
                                        {{ $administrador->apellidos }}</td>
                                    ...
                                    <td class="vm-table-td"><span
                                            class="rounded-full border border-red-500/50 px-3 py-1 text-xs font-bold text-red-200">{{ $administrador->estado }}</span>
                                    </td>
                                    <td class="vm-table-td text-right">
                                        <div class="flex justify-end gap-2">
                                            <a href="/administrador/{{ $administrador->id }}"
                                                class="vm-btn-outline !px-3 !py-1 text-xs">Ver</a>
                                            <a href="/administrador/{{ $administrador->id }}/editar"
                                                class="vm-btn-solid !px-3 !py-1 text-xs">Editar</a>
                                            <a href="/administrador/{{ $administrador->id }}/eliminar"
                            ...
                            @empty
                                <tr>
                                    <td colspan="7" class="vm-table-td py-10 text-center">No hay administradores
                                        registrados.</td>
                                </tr>
                            @endforelse
                        </tbody>
