...
<section class="vm-page-section">
    <div class="vm-container-lg">
        ...
        <div class="vm-card-form">
            <form action="/administrador/{{ $administrador->id }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                @csrf
                @method('PUT')
                <div class="grid gap-6 md:grid-cols-2">
                    <div><label class="vm-input-label" for="nombres">Nombres</label><input class="vm-input-text" id="nombres" name="nombres" value="{{ old('nombres', $administrador->nombres) }}" required></div>
                    <div><label class="vm-input-label" for="apellidos">Apellidos</label><input class="vm-input-text" id="apellidos" name="apellidos" value="{{ old('apellidos', $administrador->apellidos) }}" required></div>
