        ...
        <div class="vm-card-form">
            <form action="/administrador" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div class="grid gap-6 md:grid-cols-2">
                    ...
                    <div><label class="vm-input-label" for="imagen">Imagen</label><input class="vm-input-text" type="file" id="imagen" name="imagen" accept="image/*" required></div>
                    <div>
                    ...
                    <div>
                        ...
                        <select class="vm-input-text" id="estado" name="estado" required>
                            @foreach ($estados as $valor => $etiqueta)
                                <option value="{{ $valor }}" @selected(old('estado', 'activo') === $valor)>{{ $etiqueta }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
