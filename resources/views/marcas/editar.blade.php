    ...
    <section class="vm-page-section">
        <div class="vm-container-lg">
            <div class="vm-page-header mb-8">
                <p class="vm-header-tag">Editar marca</p>
                <h1 class="vm-header-title">{{ $marca->nombre }}</h1>
                <p class="vm-header-desc">Actualiza el registro seleccionado.</p>
            </div>
            @if ($errors->any())
            ...
            <div class="vm-card-form">
                <form action="/marcas/{{ $marca->id }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                    @csrf @method('PUT')<div class="grid gap-6 md:grid-cols-2">
                        <div><label class="vm-input-label" for="nombre">Nombre</label><input class="vm-input-text"
                                id="nombre" name="nombre" value="{{ old('nombre', $marca->nombre) }}" required></div>
                        <div><label class="vm-input-label" for="imagen">Imagen</label>
                            ...
                            <p class="mt-2 text-xs text-gray-400">{{ $marca->imagen ?? 'Sin imagen actual' }}</p>
                        </div>
                    </div>
